...
Заказчик - образовательная организация, проводящая подготовку к соревнованиям 
...
- персональные данные обучающихся хранятся на серверах за пределами РФ, что 
противоречит ФЗ 152.
...
- организует приём студентов на курс;
...
- надежное и безопасное хранение персональных данных, предоставленных решений и 
...
настроенной поверх неё системы сборки и разворачивания кода: gitlab, jenkins и 
...
Наиболее гибкий, но и наиболее продолжительный по времени и объёмный по затратам 
вариант. Уменьшить time to market тут бы позволила разработка минимальной версии 
продукта (MPV).
...
проекта продемонстрировать паттерны работы с производительностью, 
отказоустойчивостью и пр. мы просто отметим, что такой вариант есть и 
...
атрибутам качества, критическим сценариям. Рассмотрены риски всех вариантов. 
...
Система построена на основе микросервисной архитектуры и её контейнеры могут 
быть развёрнуты на любой из систем оркестрации контейнеров.

![](./docs/dicisions/ADR-0004/dsl/system/export/structurizr-1-Contestsystem-Container.svg)


Все *контейнеры-сервисы* являются stateless:
- Balancer
...
Все *контейнеры-сервисы* могут быть запущены в любом количестве, которое требует 
ситуация и технические возможности. 

Все *контейнеры с данными* являются statefull:
...
Все *контейнеры с данными* должны запускать без использования оркестраторов
контейнеров. Либо на "голом железе" либо в полновесных виртуальных машинах. 
...
- описали схему развёртывания и произвели выбор конкретных технологийю

На уровень проектирования кода мы не опускались, данные решения могут быть 
делегированы конкретным командам разработчиков, создающих тот или иной сервис. 
Решения принятые на уровне проектирования кода, при
...
В будущем в систему за счет доработки существующих сервисов или создания новых
